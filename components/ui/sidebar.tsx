...
export function SidebarContent({ children, className }: { children: React.ReactNode; className?: string }) {
  return <div className={cn("flex-1 overflow-auto p-3", className)}>{children}</div>
}
...
export function SidebarGroupLabel({ children, className }: { children: React.ReactNode; className?: string }) {
  ...
  return <div className={cn("mb-2 px-2 text-xs font-semibold text-muted-foreground", className)}>{children}</div>
}
...
export function SidebarInset({ children, className }: { children: React.ReactNode; className?: string }) {
  ...
  return (
    <div className={cn("flex-1 transition-all duration-300 ease-in-out", expanded ? "ml-64" : "ml-16", className)}>
      {children}
    </div>
  )
}
...
>(({ asChild = false, isActive = false, variant = "default", size = "default", tooltip, className, ...props }, ref) => {
  ...
  // When asChild is true, we need to handle the component differently
  if (asChild) {
    return (
      <Slot
        ref={ref}
